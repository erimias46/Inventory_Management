...
if ($result) {
    ...
    if ($row) {
        ...
        $sale_cosmetics = ($module['salecosmetics'] == 1) ? true : false;
        $deliverysalecosmetics = ($module['deliverysalecosmetics'] == 1) ? true : false;
        $deletesalecosmetics = ($module['deletesalecosmetics'] == 1) ? true : false;
    } else {
                                ...
                                <div class="overflow-hidden">
                                    <table id="zero_config"
                                        ...
                                        <tbody>
                                            ...
                                            if (!$customer) {
                                                $sql = "SELECT * 
                                                FROM cosmetics_delivery
                                                WHERE DATE(sales_date) >= '$from_date' 
                                                AND DATE(sales_date) <= '$to_date'
                                                ORDER BY sales_id DESC";
                                            } else {
                                                $sql = "SELECT * FROM cosmetics_delivery WHERE DATE(sales_date) >= '$from_date' AND DATE(sales_date) <= '$to_date' AND {$customer} ORDER BY sales_id DESC";
                                            }
                                            ...
                                            ?>
                                                ...
                                                <!-- Modal for exchanging cosmetics -->
                                                        ...
                                                        <form method="POST" class="overflow-y-auto">
                                                            <div class="px-4 py-8">
                                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                                    ...
                                                                    $sql8 = "SELECT * from cosmetics_delivery WHERE sales_id = " . $row['sales_id'];
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <input type="text" name="cosmetics_name" class="form-input" required value="<?php echo $row['cosmetics_name'] ?>">
                                                                    </div>
                                                            ...
                                                            <div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
                                                                ...
                                                                <button name="exchange_cosmetics" type="submit" class="btn bg-success text-white">Delivery</button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
...
if (isset($_POST['add_data'])) {
    ...
    if (!empty($cosmetics_name) && !empty($size)) {
        ...
        if ($method == 'delivery') {
            ...
            $add_sales = "INSERT INTO `sales`(`cosmetics_id`, `size_id`, `cosmetics_name`, `size`, `price`, `cash`, `bank`, `method`, `sales_date`, `update_date`, `quantity`, `user_id`,`bank_id`,`bank_name`,`status`) 
            ...
            $add_cosmetics_log = "INSERT INTO `sales_log`(`cosmetics_id`, `size_id`, `cosmetics_name`, `size`, `price`, `cash`, `bank`, `method`, `sales_date`, `update_date`, `quantity`, `user_id`, `status`) 
        ...
        $message = "cosmetics is going out for Delivery \n";
