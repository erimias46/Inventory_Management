                ...
                <div class="card">
                    <div class="card-header">
                        <div class="flex justify-between items-center">
                            <h4 class="text-slate-900 dark:text-slate-200 text-lg font-medium">Purchase Without VAT</h4>
                            <div>
                            ...
                            <?php if ($addButtonVisible) : ?>
                                ...
                                <a href="export.php?type=sales_withoutvat_purchase&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>"
                                    class="btn btn-sm rounded-full bg-success/25 text-success hover:bg-success hover:text-white">
                                ...
                                <div class="overflow-hidden">
                                    <table id="zero_config"
                                        ...
                                        <tbody>
                                            ...
                                            if (!$customer) {
                                                $sql = "SELECT * 
                                                FROM sales_withoutvat_purchase 
                                                WHERE DATE(sales_date) >= '$from_date' 
                                                AND DATE(sales_date) <= '$to_date'
                                                ORDER BY sales_date DESC";
                                            } else {
                                                $sql = "SELECT * FROM sales_withoutvat_purchase WHERE DATE(sales_date) >= '$from_date' AND DATE(sales_date) <= '$to_date' AND {$customer} ORDER BY sales_date DESC";
                                            }
                                            ...
                                            <tr
                                                ...
                                                <td
                                                    ...
                                                    <a id="del-btn" href="remove.php?id=<?php echo $row['sales_id'];?>&from=sales_withoutvat_purchase"
                                                    ...
                                                    <form method="POST">
                                                        <div class="px-4 py-8 overflow-y-auto">
                                                            ...
                                                            <div class="flex gap-2 justify-between">
                                                                <div class="mb-3">
                                                                    <label
                                                                        class="text-gray-800 text-sm font-medium inline-block mb-2">Purchase Price</label>
                                                        ...
                                                        <div
                                                            ...
                                                            <button name="update_purchase" type="submit"
                                                                class="btn bg-success text-white">Edit Purchase</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
                                ...
                                <button name="add_data" type="submit"
                                    class="py-2.5 px-4 inline-flex justify-center items-center gap-2 rounded bg-success hover:bg-success-600 text-white">Add Purchase</button>
                            </div>
                        </form>
                    </div>
                </div>
...
if (isset($_POST['update_purchase'])) {
    ...
    echo $customer;
    echo $description;
    $purchase_update = "UPDATE `sales_withoutvat_purchase` SET `customer`='$customer',`sale_price`='$sales_price' , `sales_date`='$date', `update_date`='$date' ,`description`='$description'
                 WHERE `sales_id` = '$sales_id'"; 
    $result_add = mysqli_query($con, $purchase_update);

    if ($result_add) {
        echo "<script>window.location = 'action.php?status=success&redirect=purchase_without.php'; </script>";
    } else {
        echo "<script>window.location = 'action.php?status=error&redirect=purchase_without.php'; </script>";
    }
}
...
if (isset($_POST['add_data'])) {
    ...
    $add_data = "INSERT INTO `sales_withoutvat_purchase`(`customer`, `sale_price`, `sales_date`, `update_date`, `description`) 
...
if (isset($_POST['filter'])) {
    ...
    $to = $_POST['to_date'];
    $customer = $_POST['customer'];
    echo "<script>window.location = 'purchase_without.php?from=$from&to=$to&customer=$customer'; </script>";
}
