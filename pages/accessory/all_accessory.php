                                ...
                                <div class="min-w-full inline-block align-middle">
                                    <div class="overflow-hidden">
                                        <table id="zero_config" data-order='[[ 0, "dsc" ]]' class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody>
                                                <?php
                                                $sql = "SELECT * FROM accessory ORDER BY created_at DESC";
                                                $result22 = mysqli_query($con, $sql);
                                                $previousName = '';
                                                $groupId = 0;
                                                while ($row = mysqli_fetch_assoc($result22)) {
                                                    // Increment groupId for each unique group of accessory_name
                                                    if ($previousName != $row['accessory_name']) {
                                                        $groupId++; // Unique ID for each group
                                                        $previousName = $row['accessory_name'];
                                                ?>
                                                    ...
                                                    <tr class="group-<?php echo $groupId; ?> hidden group-row">
                                                        ...
                                                        <td class="px-2 py-2.5 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                                            <?php if ($deleteButtonVisible) : ?>
                                                                <a id="del-btn" href="api/remove.php?id=<?php echo $row['id']; ?>&from=accessory" class="btn bg-danger/25 text-danger hover:bg-danger hover:text-white btn-sm rounded-full">
                                                                    <i class="mgc_delete_2_line text-base me-2"></i> Delete
                                                                </a>
                                                            <?php endif; ?>
                                                            <?php if ($updateButtonVisible) : ?>
                                                                <a id="edit-btn" href="edit_accessory.php?id=<?php echo $row['id']; ?>" class="btn bg-warning/25 text-warning hover:bg-warning hover:text-white btn-sm rounded-full">
