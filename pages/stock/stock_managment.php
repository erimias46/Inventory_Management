...
<body>
    ...
    <div class="flex wrapper">
        ...
        <div class="page-content">
                                    ...
                                    <table id="zero_config" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Width</th>
                                                <th>Height</th>
                                                <th>Care Lamination</th>
                                                <th>Total Lamination</th>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-700 dark:even:bg-slate-800">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                                        <?php echo $row['stock_quantity2']; ?>
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium <?php echo ($row['dangerzone'] > $row['stock_quantity2']) ? 'text-red-600' : 'text-gray-800 dark:text-gray-200'; ?>">
                                                        <?php echo $row['dangerzone']; ?>
                                                    </td>


                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $row['width']; ?></td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $row['height']; ?></td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $row['care_lamination']; ?></td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $row['total_lamination']; ?></td>
                                                ...
                                                <div id="edit<?= $row['stock_id'] ?>" class="w-full h-full fixed top-0 left-0 z-50 transition-all duration-500 hidden">
                                                    <div class="fc-modal-open:mt-7 fc-modal-open:opacity-100 fc-modal-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
                                                        <div class="flex justify-between items-center py-2.5 px-4 border-b dark:border-gray-700">
                                                            <h3 class="font-medium text-gray-800 dark:text-white text-lg">Edit Stock</h3>
                                                        ...
                                                        <form method="POST">
                                                            <div class="px-4 py-8 overflow-y-auto">
                                                                ...
                                                                <div class="mb-3">
                                                                    <label for="categoryEditSelect" class="text-gray-800 text-sm font-medium inline-block mb-2">Category</label>
                                                                    <select name="catagory" id="categoryEditSelect" class="form-select">
                                                                        <option value="">Select</option>
                                                                        <option value="lamination" <?php if ($row['catagory'] == 'lamination') echo 'selected'; ?>>Lamination</option>
                                                                        <option value="paper" <?php if ($row['catagory'] == 'paper') echo 'selected'; ?>>Paper</option>
                                                                    </select>
                                                                </div>
                                                                <div id="editAdditionalFields">
                                                                    <?php if ($row['catagory'] == 'lamination') : ?>
                                                                        <div class="mb-3">
                                                                            <label class="text-gray-800 text-sm font-medium inline-block mb-2">Width</label>
                                                                            <input type="text" name="width" class="form-input" value="<?php echo $row['width']; ?>" required>
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label class="text-gray-800 text-sm font-medium inline-block mb-2">Height</label>
                                                                            <input type="text" name="height" class="form-input" value="<?php echo $row['height']; ?>" required>
                                                                        </div>
                                                                    <?php endif; ?>
                                                                </div>
                                                            </div>
                    ...
                    <div class="fc-modal-open:mt-7 fc-modal-open:opacity-100 fc-modal-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto  bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
                        ...
                        <form method="POST">
                            <div class="px-4 py-8 overflow-y-auto">
                                ...
                                <div class="mb-3">
                                    <label for="" class="text-gray-800 text-sm font-medium inline-block mb-2">Category</label>
                                    <select name="catagory" id="categorySelect" class="form-select">
                                        <option value=""> Select</option>
                                ...
                                <div id="additionalFields"></div> <!-- Container for additional fields -->
                            </div>
                            <div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
                                <button class="py-2 px-5 inline-flex justify-center items-center gap-2 rounded dark:text-gray-200 border dark:border-slate-700 font-medium hover:bg-slate-100 hover:dark:bg-slate-700 transition-all" data-fc-dismiss type="button">Close
                                </button>
            ...
            <script>
                document.getElementById('categoryEditSelect').addEventListener('change', function() {
                    var editAdditionalFields = document.getElementById('editAdditionalFields');
                    editAdditionalFields.innerHTML = ''; // Clear previous fields

                    if (this.value === 'lamination' || this.value === 'banner') {
                        var widthField = document.createElement('div');
                        widthField.className = 'mb-3';
                        widthField.innerHTML = `
                <label class="text-gray-800 text-sm font-medium inline-block mb-2">Width</label>
                <input type="text" name="width" class="form-input" required>
            `;
                        editAdditionalFields.appendChild(widthField);

                        var heightField = document.createElement('div');
                        heightField.className = 'mb-3';
                        heightField.innerHTML = `
                <label class="text-gray-800 text-sm font-medium inline-block mb-2">Height</label>
                <input type="text" name="height" class="form-input" required>
            `;
                        editAdditionalFields.appendChild(heightField);
                    }
                });
            </script>
            ...
            <script>
                ...
                <input type="text" name="width" class="form-input" required>
            `;
                        additionalFields.appendChild(widthField);
                ...
                <input type="text" name="height" class="form-input" required>
            `;
                        additionalFields.appendChild(heightField);
                    }
                });
            </script>
...
if (isset($_POST['add_stock'])) {
    ...
    if ($catagory == 'lamination') {
        ...
        $care_lamination = $width * $height;
        $total_lamination = $care_lamination * $stock_quantity * $ratio;

        $add_stock = "INSERT INTO stock(stock_type, stock_quantity,ratio,stock_quantity2,dangerzone,catagory,width,height,care_lamination,total_lamination) 
                    VALUES ('$stock_type', '$stock_quantity','$ratio','$stock_quantity2','$stock_danger','$catagory','$width','$height','$care_lamination','$total_lamination')";
    } else if ($catagory == 'banner') {
        ...
        $add_stock = "INSERT INTO stock(stock_type, stock_quantity,ratio,stock_quantity2,dangerzone,catagory) 
                    VALUES ('$stock_type', '$stock_quantity','$ratio','$stock_quantity2','$stock_danger','$catagory')";
    }
...
if (isset($_POST['plus_stock'])) {
    ...
    $total_lamination = $row['total_lamination'];
    ...
    if ($catagory == 'lamination') {
        ...
        $total_lamination = $total_lamination + ($care_lamination * $plus_stock_quantity * $ratio);


        $stock_quantity2 =  $stock_quantity + $plus_stock_quantity;
        ...
        if ($result_log) {
            $stock_update = "UPDATE `stock` SET `stock_quantity2`='$stock_quantity2',`stock_quantity`='$stock_quantity',`total_lamination` = '$total_lamination' WHERE `stock_id` = '$stock_id'";
...
if (isset($_POST['update_stock'])) {
    ...
    if ($catagory == 'lamination') {
        ...
        $stock_update = "UPDATE `stock` SET `stock_type`='$stock_type', `stock_quantity`='$stock_quantity',`ratio`='$ratio',`stock_quantity2`='$stock_quantity2',`dangerzone` = '$dangerzone',`catagory` = '$catagory',`width` = '$width',`height` = '$height',`care_lamination` = '$care_lamination',`total_lamination` = '$total_lamination' WHERE `stock_id` = '$stock_id'";
    } else {
        $stock_update = "UPDATE `stock` SET `stock_type`='$stock_type', `stock_quantity`='$stock_quantity',`ratio`='$ratio',`stock_quantity2`='$stock_quantity2',`dangerzone` = '$dangerzone',`catagory` = '$catagory' WHERE `stock_id` = '$stock_id'";
    }
