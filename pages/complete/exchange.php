                                        ...
                                        <table id="zero_config" data-order='[[ 1, "dsc" ]]' class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="p-2.5 text-left text-xs font-medium text-gray-500 uppercase">complete Name</th>
                                            ...
                                            <tbody>
                                                <?php
    // Fetch all rows from exchange_complete ordered by id
    $sql = "SELECT * FROM exchange_complete ORDER BY id DESC";
    $result = mysqli_query($con, $sql);

    while ($row = mysqli_fetch_assoc($result)) {
        
        // Fetch details for before_sale_id
        $sql = "SELECT * FROM complete_sales WHERE sales_id = ".$row['before_sale_id'];
        $result2 = mysqli_query($con, $sql);
        if ($row2 = mysqli_fetch_assoc($result2)) {
            // If found, extract the values
            $before_date = $row2['sales_date'];
            $before_complete_name = $row2['complete_name'];
            $before_size = $row2['size'];
            $before_price = $row2['price'];
        } else {
            // If not found, skip or handle it (e.g., set default values)
            $before_date = $before_complete_name = $before_size = $before_price = 'Not found';
        }

        // Fetch details for after_sale_id
        $sql = "SELECT * FROM complete_sales WHERE sales_id = ".$row['after_sale_id'];
        $result2 = mysqli_query($con, $sql);
        if ($row2 = mysqli_fetch_assoc($result2)) {
            // If found, extract the values
            $after_date = $row2['sales_date'];
            $after_complete_name = $row2['complete_name'];
            $after_size = $row2['size'];
            $after_price = $row2['price'];
        } else {
            // If not found, skip or handle it (e.g., set default values)
            $after_date = $after_complete_name = $after_size = $after_price = 'Not found';
        }
...
?>
                                                    ...
                                                    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-700 dark:even:bg-slate-800">
                                                        ...
                                                        <td class="px-2.5 py-2 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $before_complete_name; ?></td>
                                                        ...
                                                        <td class="px-2.5 py-2 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $after_complete_name; ?></td>
