...
if ($result) {
    ...
    if ($row) {
        ...
        $sale_top = ($module['saletop'] == 1) ? true : false;
        $editsaletop = ($module['editsaletop'] == 1) ? true : false;
        $deletesaletop = ($module['deletesaletop'] == 1) ? true : false;
        $exchangesaletop = ($module['exchangesaletop'] == 1) ? true : false;
        $refundsaletop = ($module['refundsaletop'] == 1) ? true : false;
    } else {
                                            ...
                                            ?>
                                                <tr
                                                    ...
                                                    <td
                                                        ...
                                                        <?php if ($deletesaletop) : ?>

                                                            <a id="del-btn"
                                                                href="api/remove.php?id=<?php echo $row['sales_id']; ?>&from=top_sales"
                                                        ...
                                                        <?php if ($refundsaletop) : ?>

                                                            <a id="del-btn"
                                                                href="api/refund.php?id=<?php echo $row['sales_id']; ?>&from=top_sales"
                                                ...
                                                <div id="exchange<?= $row['sales_id'] ?>" class="w-full h-full fixed top-0 left-0 z-50 transition-all duration-500 hidden">
                                                    <div class="fc-modal-open:mt-7 fc-modal-open:opacity-100 fc-modal-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-md sm:w-full m-3 sm:mx-auto bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
                                                        <div class="flex justify-between items-center py-2.5 px-4 border-b dark:border-gray-700">
                                                            <h3 class="font-medium text-gray-800 dark:text-white text-lg">Exchange top</h3>
                                                        ...
                                                        <form method="POST" class="overflow-y-auto">
                                                            <div class="px-4 py-8">
                                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <select name="top_name" class="search-select">
                                                                            <?php
                                                                            $sql3 = "SELECT * FROM top GROUP BY top_name ORDER BY top_name ASC";
                                                                            $result3 = mysqli_query($con, $sql3);
                                                                            if (mysqli_num_rows($result3) > 0) {
                                                                                while ($row3 = mysqli_fetch_assoc($result3)) { ?>
                                                                                    <option value="<?= $row3['top_name'] ?>"><?= $row3['top_name'] ?></option>
                                                                            <?php }
                                                                            }
                                                                            ?>
                                                                        </select>
                                                                    ...
                                                                    <div>
                                                                        <label class="text-gray-800 text-sm font-medium inline-block mb-2">top Size</label>
                                                                        <select name="size" class="search-select">
                                                                            <?php
                                                                            $sql4 = "SELECT * FROM topdb";
                                                            ...
                                                            <div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
                                                                ...
                                                                <button name="exchange_top" type="submit" class="btn bg-success text-white">Exchange</button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
                                                        ...
                                                        <form method="POST" class="overflow-y-auto">
                                                            <div class="px-4 py-8">
                                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                                    ...
                                                                    <?php
                                                                    $sql2 = "SELECT * FROM top_sales WHERE sales_id = " . $row['sales_id'];
                                                                    ...
                                                                    <div>
                                                                        <label class="text-gray-800 text-sm font-medium inline-block mb-2">top Name</label>
                                                                        <select name="top_name" class="search-select" id="top_name_select" disabled>
                                                                            <?php
                                                                            $sql3 = "SELECT * FROM top GROUP BY top_name ORDER BY top_name ASC";
                                                                            $result3 = mysqli_query($con, $sql3);
                                                                            if (mysqli_num_rows($result3) > 0) {
                                                                                while ($row3 = mysqli_fetch_assoc($result3)) { ?>
                                                                                    <option value="<?= $row3['top_name'] ?>"
                                                                                        <?php
                                                                                        // Check if the current top_name should be selected
                                                                                        if (isset($row['top_name']) && $row['top_name'] == $row3['top_name']) {
                                                                                            echo "selected";
                                                                                        }
                                                                                        ?>>
                                                                                        <?= $row3['top_name'] ?>
                            ...
                            <div class="px-4 py-8 overflow-y-auto">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="top_name" class="search-select" required onchange="fetchPrice()">

                                            <?php

                                            $sql = "SELECT * FROM top GROUP BY top_name ORDER BY top_name ASC";
                                            $result = mysqli_query($con, $sql);
                                            while ($row = mysqli_fetch_assoc($result)) {
                                            ?>
                                                <option value="<?php echo $row['top_name'] ?>" <?php
                                                                                                    if (isset($top_name)) {
                                                                                                        if ($row['top_name'] == $top_name) {
                                                                                                            echo "selected";
                                                                                                        }
                                                                                                    }
                                                                                                    ?>>
...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE top_sales SET  price = '$price', cash = '$cash', bank = '$bank', update_date = '$date', user_id = '$user_id', bank_id = '$bank_id', bank_name = '$bank_name' WHERE sales_id = '$sales_id'";
    $result = mysqli_query($con, $sql);
...
if (isset($_POST['add_data'])) {
    ...
    if (!empty($top_name) && !empty($size)) {
        ...
        if ($method == 'delivery') {
            ...
            $status = "pending";
            $sql = "INSERT into top_delivery (top_id, size_id, top_name, size, price, cash, bank, method, sales_date, update_date, quantity, user_id,bank_id,bank_name,status)
            VALUES ('$top_id', '$size_id', '$top_name', '$size', '$price', '$cash', '$bank', '$method', '$date', '$date', '$quantity', '$user_id','$bank_id','$bank_name','$status')";
            $result = mysqli_query($con, $sql);
...
if (isset($_POST['filter'])) {
    ...
    echo "<script>window.location = 'sale_top.php?from=$from&to=$to&customer=$customer'; </script>";
}
