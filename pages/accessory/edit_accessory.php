...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE accessory SET accessory_name='$accessory_name', size='$size', type='$type', price='$price', quantity='$quantity', image='$image_path' WHERE id='$id'";
    $result = mysqli_query($con, $sql);

    if ($result) {
        echo "<script>window.location = 'action.php?status=success&redirect=all_accessory.php';</script>";
        ...
        echo "<script>window.location = 'action.php?status=error&message=Error updating accessory in the database.&redirect=all.php';</script>";
    }
}
                        ...
                        <div class="p-6">
                            ...
                            <form method="post" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="accessory_name" class="form-input" list="accessory_types" required value="<?php echo $accessory_name ?>">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="type" class="search-select" required>
                                        ...
                                        $sql = "SELECT * FROM accessory_type_db";
                                        $result = mysqli_query($con, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                        ?>
