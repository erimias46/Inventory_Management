                    ...
                    <div class="p-6">
                        <form method="POST">
                            <div class="flex flex-wrap">
                                ...
                                <div class="me-3">
                                    ...
                                    <select class="search-select" name="type">
                                        ...
                                        <option value="" <?php echo isset($_GET['type']) && $_GET['type'] === '' ? 'selected' : ''; ?>>All Client</option>
                                        <?php
                                        $sql = "SELECT * FROM bank GROUP BY client";
                                        $result = mysqli_query($con, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            // Check if the current client should be selected
                                            $selected = isset($_GET['type']) && $_GET['type'] === $row['client'] ? 'selected' : '';
                                        ?>
                            ...
                            <div class="flex justify-end">
                                <a href="bank.php" class="btn btn-sm bg-danger text-white rounded-full me-2">
                ...
                <div class="card mt-3">
                    <div class="card-header">
                        <div class="flex gap-2 justify-between">
                            <h3 class="text-lg font-bold text-gray-800 dark:text-white"> Payment Report </h3>
                            <?php
                            ...
                            if (isset($from) and isset($to))
                                $link = "export.php?file=bank&from=$from&to=$to&client=$client";
                            else
                                $link = "export.php?file=bank&client=$client";
                            ?>
                                ...
                                <div class="overflow-hidden">
                                    <table id="myTable"
                                        ...
                                        <tbody>
                                            ...
                                            if (!$type) {
                                                $sql = "SELECT * FROM bank WHERE DATE(date) >= '$from_date' AND DATE(date) <= '$to_date' order by bank_id desc";
                                            } else {
                                                $sql = "SELECT * FROM bank WHERE DATE(date) >= '$from_date' AND DATE(date) <= '$to_date' AND {$type} order by bank_id desc";
                                            }
                                            $result = mysqli_query($con, $sql);
                                            while ($row = mysqli_fetch_assoc($result)) {
                                            ?>
                                                <tr
                                                    ...
                                                    <td
                                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                                        <?php echo $row['reference_number']; ?></td>
                                                    <td
                                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
...
if (isset($_POST['filter'])) {
    ...
    $to = $_POST['to_date'];
    $type = $_POST['type'];
    echo "<script>window.location = 'bank.php?from=$from&to=$to&type=$type'; </script>";
}
