                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="text-end">
                                ...
                                <button class="btn btn-sm bg-danger text-white rounded-full" onclick="window.location.href='add_top.php'">Add top</button>
...
<script>
    ...
    function selectOption(value) {
        const input = document.getElementById('top_name');
        ...
        fetch(`get_top_image.php?top_name=${encodeURIComponent(value)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.image_path) {
                    preview.innerHTML = `<img src="${data.image_path}" alt="top Image" />`;
                } else {
                    preview.innerHTML = `<img src="../../../include/uploads/defaulttop.jpg" alt="Default top Image" />`;
                }
            })
            .catch(() => {
                preview.innerHTML = `<img src="../../../include/uploads/defaulttop.jpg" alt="Default top Image" />`;
...
<style>
    ...
    <div class="image-preview" id="imagePreview">
        <?php
        if (isset($top_name)) {
            $sql = "SELECT image FROM top WHERE top_name='" . mysqli_real_escape_string($con, $top_name) . "'";
            $result = mysqli_query($con, $sql);
            $row = mysqli_fetch_assoc($result);

            if ($row && file_exists('../../../include/' . $row['image'])) {
                $image_path = '../../../include/' . $row['image'];
                echo "<img src='$image_path' alt='top Image' />";
            } else {
                echo "<img src='../../../include/uploads/defaulttop.jpg' alt='Default top Image' />";
            }
        } else {
            echo "<img src='../../../include/uploads/defaulttop.jpg' alt='Default top Image' />";
        }
