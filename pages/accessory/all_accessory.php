...
<head>
    ...
    if ($result) {
        ...
        if ($row) {
            ...
            $updateButtonVisible = ($module['editaccessory'] == 1) ? true : false;
            $deleteButtonVisible = ($module['deleteaccessory'] == 1) ? true : false;
        } else {
                                ...
                                <div class="min-w-full inline-block align-middle">
                                    <div class="overflow-hidden">
                                        <table id="zero_config" data-order='[[ 0, "dsc" ]]' class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody>
                                                <?php
                                                $sql = "SELECT * FROM accessory ORDER BY accessory_name, created_at DESC";
                                                ...
                                                // Store all unique accessory names first
                                                $uniqueNames = [];
                                                $colorMap = [];
                                                $tempResults = [];
                                                ...
                                                while ($row = mysqli_fetch_assoc($result22)) {
                                                    $tempResults[] = $row;
                                                    if (!in_array($row['accessory_name'], $uniqueNames)) {
                                                        $uniqueNames[] = $row['accessory_name'];
                                                        $colorIndex = count($uniqueNames) - 1;
                                                        $colorMap[$row['accessory_name']] = $colors[$colorIndex % count($colors)];
                                                    }
                                                }
                                                ...
                                                foreach ($tempResults as $row) {
                                                    ...
                                                    $color = $colorMap[$accessoryName];

                                                    // Start new group
                                                    if ($currentName !== $accessoryName) {
                                                        $groupId++;
                                                ...
                                                ?>
                                                        ...
                                                        <tr style="background-color: <?php echo $color; ?>;" class="cursor-pointer hover:opacity-90">
                                                            ...
                                                            <td class="px-2 py-2.5 text-sm font-medium text-gray-900"><?php echo $accessoryName; ?></td>
                                                    ...
                                                    <tr class="group-<?php echo $groupId; ?> hidden group-row" style="background-color: <?php echo $color; ?>;">
                                                        ...
                                                        <td class="px-2 py-2.5 whitespace-nowrap text-sm font-medium text-gray-900">
                                                            <?php if ($deleteButtonVisible) : ?>
                                                                <a id="del-btn" href="api/remove.php?id=<?php echo $row['id']; ?>&from=accessory"
                                                            ...
                                                            <?php if ($updateButtonVisible) : ?>
                                                                <a id="edit-btn" href="edit_accessory.php?id=<?php echo $row['id']; ?>"
