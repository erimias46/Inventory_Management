                                ...
                                <div class="min-w-full inline-block align-middle">
                                    <div class="overflow-hidden">
                                        <table id="zero_config" data-order='[[ 1, "dsc" ]]' class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody>
                                                <?php
    // Fetch all rows from exchange_top ordered by id
    $sql = "SELECT * FROM exchange_top ORDER BY id DESC";
    $result = mysqli_query($con, $sql);

    while ($row = mysqli_fetch_assoc($result)) {
        
        // Fetch details for before_sale_id
        $sql = "SELECT * FROM top_sales WHERE sales_id = ".$row['before_sale_id'];
        $result2 = mysqli_query($con, $sql);
        if ($row2 = mysqli_fetch_assoc($result2)) {
            // If found, extract the values
            $before_date = $row2['sales_date'];
            $before_top_name = $row2['top_name'];
            $before_size = $row2['size'];
            $before_price = $row2['price'];
        } else {
            // If not found, skip or handle it (e.g., set default values)
            $before_date = $before_top_name = $before_size = $before_price = 'Not found';
        }

        // Fetch details for after_sale_id
        $sql = "SELECT * FROM top_sales WHERE sales_id = ".$row['after_sale_id'];
        $result2 = mysqli_query($con, $sql);
        if ($row2 = mysqli_fetch_assoc($result2)) {
            // If found, extract the values
            $after_date = $row2['sales_date'];
            $after_top_name = $row2['top_name'];
            $after_size = $row2['size'];
            $after_price = $row2['price'];
        } else {
            // If not found, skip or handle it (e.g., set default values)
            $after_date = $after_top_name = $after_size = $after_price = 'Not found';
        }
...
?>
                                                    ...
                                                    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-700 dark:even:bg-slate-800">
                                                        ...
                                                        <td class="px-2.5 py-2 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $before_top_name; ?></td>
                                                        ...
                                                        <td class="px-2.5 py-2 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200"><?php echo $after_top_name; ?></td>
    ...
    if (isset($_POST['update'])) {
        foreach ($_POST['update'] as $update_id) {
            ...
            if ($error == '2') {
                ...
                $update = "UPDATE `top` SET `quantity`= '$quantity' WHERE top_name = '$top_name' AND size = '$size'";
