                                ...
                                <div class="min-w-full inline-block align-middle">
                                    <div class="overflow-hidden">
                                        <table id="zero_config" data-order='[[ 0, "dsc" ]]' class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody>
                                                <?php
                                                $sql = "SELECT * FROM top ORDER BY created_at DESC";
                                                $result22 = mysqli_query($con, $sql);
                                                $previousName = '';
                                                $groupId = 0;
                                                while ($row = mysqli_fetch_assoc($result22)) {
                                                    // Increment groupId for each unique group of top_name
                                                    if ($previousName != $row['top_name']) {
                                                        $groupId++; // Unique ID for each group
                                                        $previousName = $row['top_name'];
                                                ?>
