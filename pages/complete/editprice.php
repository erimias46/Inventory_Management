...
if (isset($_POST['update'])) {
    ...
    $id = $_GET['id']; // Assuming `complete_id` is passed for identifying the record
    ...
    $sql = "UPDATE complete SET buy_price = '$buy_price', price = '$selling_price',image='$image_path' WHERE complete_name = '$complete_name'";
    $result = mysqli_query($con, $sql);

    if ($result) {
        echo "<script>window.location = 'action.php?status=success&redirect=type_complete.php';</script>";

        //send notification

        $message = " complete Price Updated:\n";
        $message .= "complete Name: " . $complete_name . "\n";
        $message .= "Selling Price: " . $selling_price . "\n";
        $message .= "Buy Price: " . $buy_price . "\n";


        $subject = "complete Updated";
                    ...
                    <div class="card">
                        ...
                        <div class="p-6">
                            ...
                            <form method="post" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                ...
                                <div class="mb-3">
                                    <label class="text-gray-800 text-sm font-medium inline-block mb-2" for="complete names">complete Name</label>
                                    <input type="text" name="complete_name" id="complete_name" value="<?php if (isset($complete_name)) echo  $complete_name ?>" class="form-input" list="complete_types" required>
